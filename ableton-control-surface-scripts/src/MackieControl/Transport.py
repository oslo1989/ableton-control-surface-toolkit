...
class Transport(MackieControlComponent):
    ...
    def destroy(self):
        ...
        self.application().view.remove_is_view_visible_listener(
            "Session",
            self._Transport__on_session_is_visible_changed,
        )
    ...
    def on_update_display_timer(self):
        if self._Transport__transport_repeat_delay > 2:
            base_acceleration = 1 if self.alt_is_pressed() else self.song().signature_numerator
                ...
                if self._Transport__forward_button_down:
                    ...
                    if not self.alt_is_pressed():
                        self._Transport__fast_forward(
                            base_acceleration + max(1, old_div(self._Transport____fast_forward_counter, 4)),
                        )
                    else:
                ...
                if self._Transport____rewind_button_down:
                    ...
                    if not self.alt_is_pressed():
                        self._Transport__rewind(
                            base_acceleration + max(1, old_div(self._Transport__fast___rewind_counter, 4)),
                        )
                    else:
    ...
    def handle_jog_wheel_rotation(self, value):
        backwards = value >= 64
        if self.control_is_pressed():
            step = 0.1 if self.alt_is_pressed() else 1.0
            amount = -(value - 64) if backwards else value
            ...
            if self.session_is_visible():
                ...
                step = max(1.0, (value - 64) / 2.0) if backwards else max(1.0, value / 2.0)
                if self.song().is_playing:
                    step *= 4.0
                ...
                if switch_id == SID_JOG_CURSOR_LEFT:
                    ...
                    if switch_id == SID_JOG_CURSOR_RIGHT:
                        ...
                        if switch_id == SID_JOG_ZOOM:
                            if value == BUTTON_PRESSED:
                                if self.session_is_visible():
                                    if self.selected_clip_slot():
                                        if self.alt_is_pressed():
                                            self.selected_clip_slot().has_stop_button = (
                                                not self.selected_clip_slot().has_stop_button
                                            )
                                        else:
                            ...
                            if switch_id != SID_JOG_SCRUB or value == BUTTON_PRESSED and self.session_is_visible():
                                if self.option_is_pressed():
        ...
        if self.session_is_visible():
            ...
            if clip_slot and clip_slot.clip:
                if clip_slot.clip.is_triggered:
                    ...
                    state = CLIP_PLAYING if clip_slot.clip.is_playing else CLIP_STOPPED
            else:
                state = CLIP_STOPPED

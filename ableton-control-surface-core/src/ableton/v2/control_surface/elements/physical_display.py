...
class DisplayElement(ControlElement):
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__} {self.display_string!r}>"
    ...
    def set_num_segments(self, num_segments):
        ...
        if not in_range(num_segments, 1, width) or width % num_segments != 0:
            raise DisplaySegmentationError(
                "Can not split display of size %d into %d segments" % (self.width, num_segments),
            )
        if num_segments != len(self._logical_segments):
            self._disconnect_segments()
            self._width_per_segment = old_div(width, num_segments)
            self._logical_segments = [
                LogicalDisplaySegment(self._width_per_segment, self.update) for _ in range(num_segments)
            ]
...
class PhysicalDisplayElement(DisplayElement, NotifyingControlElement):
    ...
    def __init__(self, *a, **k):
        self._central_resource = _DisplayCentralResource(
            root_display=self,
            on_received_callback=(self._on_central_resource_received),
            on_lost_callback=(self._on_central_resource_lost),
        )
        (super().__init__)(a, resource_type=self.nested_display_resource_factory(self), **k)
    ...
    def reset(self):
        ...
        if not self._block_messages:
            if self._message_clear_all is not None:
                ...
                self._message_to_send = tuple(
                    chain(self._message_header, self._translate_string(" " * int(self._width)), self._message_tail),
                )
            self._request_send_message()
    ...
    def _translate_char(self, char_to_translate):
        result = 63
        if char_to_translate in self._translation_table:
...
class SubDisplayElement(DisplayElement):
    def __init__(self, sub_display_slice=slice(1), parent_display=None, *a, **k):
        (super().__init__)(
            a,
            width_in_chars=slice_size(sub_display_slice, parent_display.width),
            resource_type=parent_display.nested_display_resource_factory(self),
            **k,
        )

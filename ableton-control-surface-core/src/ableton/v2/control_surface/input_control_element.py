...
from ..base import Disconnectable, Event, Signal, const, depends, liveobj_valid, nop, task
from . import midi
...
class InputSignal(Signal):
    def __init__(self, sender=None, *a, **k):
        (super().__init__)(a, sender=sender, **k)
    ...
    def disconnect(self, *a, **k):
        with self._listeners_update():
            (super().disconnect)(*a, **k)

    def disconnect_all(self, *a, **k):
        with self._listeners_update():
            (super().disconnect_all)(*a, **k)
...
class InputControlElement(NotifyingControlElement):
    ...
    def __init__(
        ...
        send_should_depend_on_forwarding=True,
    ...
    def install_connections(self, install_translation, install_mapping, install_forwarding):
        ...
        if liveobj_valid(self._parameter_to_map_to):
            self._is_mapped = install_mapping(
                self,
                self._parameter_to_map_to,
                self._mapping_feedback_delay,
                self._mapping_feedback_values(),
            )
    ...
    def send_value(self, value, force=False, channel=None):
        ...
        if force or self._force_next_send:
            ...
            if (
                not self.send_depends_on_forwarding
                or self._is_being_forwarded
                and self._send_delayed_messages_task.is_running
            ):

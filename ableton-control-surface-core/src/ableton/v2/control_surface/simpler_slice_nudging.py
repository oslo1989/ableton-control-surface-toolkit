...
class SimplerSliceNudging(EventObject):
    ...
    def _can_access_slicing_properties(self):
        return (
            liveobj_valid(self._simpler)
            and liveobj_valid(self._simpler.sample)
            and self._simpler.current_playback_mode == Live.SimplerDevice.PlaybackMode.slicing
        )
            ...
            if old_slice_time >= 0:
                ...
                if old_slice_time != new_slice_time:
                    ...
                    try:
                        ...
                        self._simpler.view.selected_slice = self._simpler.slices[
                            list(original_slices).index(old_slice_time)
                        ]

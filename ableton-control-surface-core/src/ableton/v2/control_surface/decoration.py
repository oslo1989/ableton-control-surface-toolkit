...
class LiveObjectDecorator(CompoundDisconnectable, Proxy):
    def __init__(self, live_object=None, additional_properties=None):
    ...
    def _add_non_automatable_enum_parameter(self, name, list, index):
        self._LiveObjectDecorator__decorated_parameters.append(
    ...
    def _add_switch_option(self, name, pname, labels):
        self._LiveObjectDecorator__decorated_options[pname] = DeviceSwitchOption(
            name=name,
    ...
    def _add_on_off_option(self, name, pname):
        self._LiveObjectDecorator__decorated_options[pname] = DeviceOnOffOption(
            name=name,
...
class DecoratorFactory(CompoundDisconnectable):
    ...
    def decorate(self, live_object, additional_properties=None, **k):
        if additional_properties is None:
            additional_properties = {}
        if self._should_be_decorated(live_object):
            if not self.decorated_objects.get(live_object, None):
                self.decorated_objects[live_object] = self.register_disconnectable(
                    (self._get_decorated_object)(live_object, additional_properties, **k),
                )
...
class PitchParameter(InternalParameter):
    ...
    def display_value(self):
        return f"{self._combined_value:.2f}st"
    ...
    def automation_state(self):
        ...
        if AutomationState.playing in (integer_host_automation_state, decimal_host_automation_state):

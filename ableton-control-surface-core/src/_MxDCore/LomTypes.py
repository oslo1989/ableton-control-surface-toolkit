...
def warp_markers_to_json(obj):
    property_value = obj.warp_markers
...
EXPOSED_TYPE_PROPERTIES = {
    ...
    Live.DrumChain.DrumChain: (*_CHAIN_BASE_PROPS, MFLProperty("out_note"), MFLProperty("choke_group")),
    ...
    Live.Eq8Device.Eq8Device.View: (*_DEVICE_VIEW_BASE_PROPS, MFLProperty("selected_band")),
    ...
    Live.PluginDevice.PluginDevice: (*_DEVICE_BASE_PROPS, MFLProperty("presets"), MFLProperty("selected_preset_index")),
    ...
    Live.SimplerDevice.SimplerDevice.View: (*_DEVICE_VIEW_BASE_PROPS, MFLProperty("selected_slice")),
    Live.Song.Song: (
...
def cs_base_classes():
    from _Framework import ControlElement, ControlSurfaceComponent
...
def is_lom_object(lom_object, lom_classes):
    return isinstance(lom_object, (Live.Base.Vector, cs_base_classes(), (*tuple(lom_classes), type(None))))
...
def verify_object_property(lom_object, property_name, epii_version):
    ...
    if isinstance(lom_object, cs_base_classes()):
        ...
        if not is_property_exposed_for_type(property_name, type(lom_object), epii_version):
            raise_error = True
    if raise_error:
        raise LomAttributeError(f"'{lom_object.__class__.__name__}' object has no attribute '{property_name}'")

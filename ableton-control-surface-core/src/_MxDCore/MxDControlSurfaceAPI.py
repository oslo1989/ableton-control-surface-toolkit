...
def check_has_mxd_midi_scheduler(lom_object, shown_name):
    if not old_hasattr(lom_object, "mxd_midi_scheduler"):
        raise AttributeError(f"object '{type(lom_object).__name__}' has no attribute '{shown_name}'")


class MxDControlSurfaceAPI:
    ...
    class MxDMidiOwner(namedtuple("Owner", "device_id object_id mxdcore")):
        def send_reply(self, selector, message):
            ...
            selector_str = {
                ...
                "received": "received_midi",
            }.get(selector, str(selector))
            with contextlib.suppress(Exception):
                self.mxdcore.manager.send_message(
                    self.device_id,
                    self.object_id,
                    "obj_output",
                    '"' + selector_str + '"  ' + message_str,
                )
    ...
    def object_get_control_names(self, device_id, object_id, lom_object, parameters):
        if not is_control_surface(lom_object):
            raise AttributeError(f"object '{type(lom_object).__name__}' has no attribute get_control_names")
    ...
    def object_get_control(self, device_id, object_id, lom_object, parameters):
        if not is_control_surface(lom_object):
            raise AttributeError(f"object '{type(lom_object).__name__}' has no attribute get_control")
        cs = lom_object
        name = parameters[1]
    ...
    def _get_control_or_raise(self, cs, control_or_name, command):
        if not is_control_surface(cs):
            raise AttributeError(f"object '{type(cs).__name__}' has no attribute {command}")
        if not control_or_name:
            raise AttributeError(f"control id or name required for {command}")
        if isinstance(control_or_name, string_types):
            control = cs.get_control_by_name(control_or_name)
            assert control, f"{control_or_name} is not a control of '{type(cs).__name__}'"
        else:
            control = control_or_name
            if not cs.has_control(control):
                id_str = self._mxdcore.str_representation_for_object(control)
                raise AttributeError(f"'{type(control).__name__}' ({id_str}) is not a control of '{type(cs).__name__}'")
        return control

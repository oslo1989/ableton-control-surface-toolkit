...
from .LomTypes import (
    ...
    ROOT_KEYS,
    LomAttributeError,
    LomNoteOperationError,
    LomNoteOperationWarning,
    LomObjectError,
    MFLPropertyFormats,
    data_dict_to_json,
    get_exposed_lom_types,
    get_exposed_property_info,
    get_root_prop,
    is_control_surface,
    is_cplusplus_lom_object,
    is_lom_object,
    is_object_iterable,
    verify_object_property,
)
from .LomUtils import (
    LomInformation,
    LomIntrospection,
    LomPathCalculator,
    LomPathResolver,
    is_control_surfaces_list,
    wrap_control_surfaces_list,
)
from .MxDControlSurfaceAPI import MxDControlSurfaceAPI
from .MxDUtils import StringHandler, TupleWrapper
from .NotesAPIUtils import (
    MIDI_NOTE_ATTRS,
    VALID_DUPLICATE_NOTES_BY_ID_PARAMETERS,
    midi_note_to_dict,
    verify_note_specification_requirements,
)
...
def parameter_to_bool(parameter):
    bool_value = False
    if isinstance(parameter, (int, bool)):
...
class MxDCore:
    ...
    def __init__(self, *a, **k):
        ...
        self._call_handler = {
            "get_notes": self._object_get_notes_handler,
    ...
    def set_manager(self, manager):
        ...
        manager.set_manager_callbacks(
            self.update_observer_listener,
            self.install_observer_listener,
            self.uninstall_observer_listener,
            self.update_timeable,
        )
    ...
    def _get_lom_ids_of_mapped_objects(self, lom_id):
        ...
        return (observers, remotes, modulators)
    ...
    def _set_current_lom_id(self, device_id, object_id, lom_id, type):
        ...
        device_context[CONTAINS_CS_ID_KEY] |= lom_id in self.appointed_lom_ids
    ...
    def update_device_context(self, device_id, object_id):
        ...
        if object_id not in list(self.device_contexts[device_id].keys()):
            self.device_contexts[device_id][object_id] = {
                PATH_KEY: [],
                ID_KEY: 0,
                TYPE_KEY: None,
                PROP_KEY: "",
    ...
    def obj_get_path(self, device_id, object_id, parameters):
        ...
        if len(path) == 0 and lom_object is not None:
                    ...
                    if isinstance(prop, float):
                        ...
                        if isinstance(prop, string_types):
                            ...
                            if isinstance(prop, tuple):
                                prop_info = get_exposed_property_info(
                                    type(lom_object),
                                    property_name,
                                    self.epii_version,
                                )
    ...
    def obj_call(self, device_id, object_id, parameters):
        ...
        try:
            ...
            self._print_error(device_id, object_id, f"{PARSE_ERROR}: {parameters}")
            return
        else:
        ...
        if current_object is not None:
            try:
                ...
                handler = (
                    self._call_handler[func_name]
                    if func_name in list(self._call_handler.keys())
                    else self._object_default_call_handler
                )
                ...
                try:
                    self._print_error(device_id, object_id, f"{e!s}: '{to_str(param_comps)}'")
                finally:
                    e = None
                    del e
    ...
    def obs_get_prop(self, device_id, object_id, parameter):
        self.manager.send_message(
            ...
            object_id,
            "obs_prop",
            str(self._get_current_property(device_id, object_id)),
        )
    ...
    def _object_for_id(self, device_id):
        with contextlib.suppress(Exception):
            return partial(self._get_lom_object_by_lom_id, device_id)
    ...
    def _object_get_notes_handler(self, device_id, object_id, lom_object, parameters):
        notes = lom_object.get_notes(parameters[1], parameters[2], parameters[3], parameters[4])
    ...
    def _object_get_notes_by_id_handler(self, device_id, object_id, lom_object, parameters):
        ...
        self._do_get_notes_extended(
            device_id,
            object_id,
            lom_object,
            function_name,
            dict_from_args,
            function_parameters if (not dict_from_args) else ([]),
        )
    ...
    def _do_get_notes_extended(self, device_id, object_id, lom_object, function_name, param_dict, *function_parameters):
        ...
        if param_dict is not None:
            ...
            if properties_to_return is not None:
                properties_to_return = self._sanitize_midi_note_property_list(
                    device_id,
                    object_id,
        ...
        result = self.str_representation_for_object(
            self._midi_note_vector_to_dict_output(midi_note_vector, properties_to_return),
        )
        ...
        if param_dict is not None:
            ...
            if invalid:
                ...
                self._print_warning(
                    device_id,
                    object_id,
                    "Invalid key{} provided: {}. {} will be ignored.".format(
                        "s" if multiple else "",
                        ", ".join(invalid),
                        "They" if multiple else "It",
                    ),
                )
            try:
    ...
    def _object_selected_notes_handler(self, device_id, object_id, lom_object, parameters):
        notes = lom_object.get_selected_notes()
        self.manager.send_message(device_id, object_id, "obj_call_result", self._create_notes_output(notes))
    ...
    def _sanitize_midi_note_property_list(self, device_id, object_id, property_list):
        ...
        if invalid:
            ...
            self._print_warning(
                device_id,
                object_id,
                "Invalid propert{} provided: {}. {} will be ignored.".format(
                    "ies" if multiple else "y",
                    ", ".join(invalid),
                    "They" if multiple else "It",
                ),
            )
                ...
                if property_name != "":
                    if old_hasattr(current_object, property_name):
                        ...
                        if prop_type is None:
                            ...
                            prop_value = self.str_representation_for_object(
                                prop_info.to_json(current_object) if prop_info and (prop_info.to_json) else prop,
                                mark_ids=False,
                            )
    ...
    def _observer_id_callback(self, device_id, object_id):
        ...
        current_id = 0 if current_object is None else self._get_current_lom_id(device_id, object_id)
    ...
    def _update_timeable(self, device_id, object_id, object_type, validate_change_allowed):
        ...
        if isinstance(current_object, Live.DeviceParameter.DeviceParameter):
            success = self.manager.register_timeable(
                device_id,
                object_id,
                object_type,
                current_object,

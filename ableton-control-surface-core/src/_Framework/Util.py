    ...
    def patcher(func):
        ...
        if not override:
            if old_hasattr(target, patchname):
                raise TypeError(f"Class {target.__name__} already has method {patchname}")
...
def is_matrix(iterable):
    if is_iterable(iterable):
        if len(iterable) > 0:
            return all(is_iterable(x) and len(iterable[0]) == len(x) and len(x) > 0 for x in iterable)
    return False
...
class NamedTuple:
    ...
    def _eq_dict(self):
        ...
        return reduce(
            lambda a, b: union(b, a),
            [public(c.__dict__) for c in self.__class__.__mro__],
...
class Slicer:
    def __init__(self, dimensions=1, extractor=nop, keys=(), *a, **k):
        (super().__init__)(*a, **k)
        self._keys = keys
